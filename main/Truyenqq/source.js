...
var _Sources = (() => {
  ...
  var TruyenqqInfo = {
    ...
    sourceTags: [{ text: "Raw", type: import_types2.BadgeColor.GREY }]
  };
  var Truyenqq = class {
    constructor() {
    ...
    async getHomePageSections(sectionCallback) {
      const sections = [
        App.createHomeSection({
          ...
          title: "Danh S\xE1ch Truy\u1EC7n Tranh M\u1EDBi",
      ...
      const listUpdateItems = await this.getMangaListUpdateItems("truyen-moi-cap-nhat", void 0);
      const listTopMonthItems = await this.getMangaListUpdateItems("top-thang", void 0);
      const listSuggestItems = await this.getMangaListUpdateItems("truyen-yeu-thich", void 0);
      for (const section of sections) {
        switch (section.id) {
    ...
    async getMangaListUpdateItems(category, metadata) {
      ...
      $2("div#main_homepage ul.list_grid.grid li").each((_, element) => {
        const el = $2(element);
      ...
      const hasNextPage = $2(`div .page_redirect a`).length > 0;
      return App.createPagedResults({
        results: items,
        metadata: hasNextPage ? { page: page + 1, collectedIds: Array.from(seen) } : void 0
      });
    }
    async getSearchResults(query, metadata) {
      ...
      const $2 = load(response.data);
      const results = [];
      const seenIds = new Set(collectedIds);
      $2("#main_homepage .list_grid li").each((_, element) => {
        const el = $2(element);
    ...
    async getMangaDetails(mangaId) {
      ...
      const $2 = load(response.data);
      const title = $2('h1[itemprop="name"]').first().text().trim();
      const image = $2('.book_avatar img[itemprop="image"]').first().attr("src") || "";
      const description = $2("div.story-detail-info").first().text().trim();
      const status = $2(".book_info div.txt .status.row p").last().text().trim();
      const publisher = $2("a.org").first().text().trim();
    ...
    async getChapters(mangaId) {
      ...
      const $2 = load(response.data);
      const chapters = [];
      $2(".works-chapter-list .works-chapter-item").each((_, element) => {
        const el = $2(element);
        ...
        let chapterDate = /* @__PURE__ */ new Date();
        if (dateStr) {
    ...
    async getChapterDetails(mangaId, chapterId) {
      ...
      const $2 = load(response.data);
      const pages = [];
      $2(".chapter_content img.lazy").each((_, element) => {
        const el = $2(element);

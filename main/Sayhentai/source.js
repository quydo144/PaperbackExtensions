...
var _Sources = (() => {
  ...
  // src/Sayhentai/Sayhentai.ts
  var Sayhentai_exports = {};
  __export(Sayhentai_exports, {
    Sayhentai: () => Sayhentai,
    SayhentaiInfo: () => SayhentaiInfo
  });
  ...
  var Sayhentai = class {
    ...
    async getHomePageSections(sectionCallback) {
      ...
      const listUpdateItems = await this.getMangaListUpdateItems("", void 0);
      const listManhwaItems = await this.getMangaListUpdateItems("/genre/manhwa", void 0);
      const listCompletedItems = await this.getMangaListUpdateItems("/completed", void 0);
      for (const section of sections) {
        switch (section.id) {
    ...
    async getMangaListUpdateItems(category, metadata) {
      ...
      $2(".page-item-detail").each((_, element) => {
        const el = $2(element);
      ...
      const hasNextPage = $2(".pager").find("li.active + li").length > 0;
    ...
    async getMangaDetails(mangaId) {
      ...
      const rawImage = $2(".summary_image img").first().attr("src")?.trim() || "";
      const image = rawImage ? encodeURI(rawImage) : "";
      const description = $2(".original-content").first().text().trim();
    ...
    async getChapterDetails(mangaId, chapterId) {
      ...
      const $2 = load(response.data);
      const pages = [];
      $2("#chapter_content img").each((_, element) => {
        const el = $2(element);
  ...
  return __toCommonJS(Sayhentai_exports);
})();
